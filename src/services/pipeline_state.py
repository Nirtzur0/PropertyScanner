...
from src.core.config import FUSION_MODEL_PATH, VECTOR_INDEX_PATH, VECTOR_METADATA_PATH
...
class PipelineStateService:
    ...
    def snapshot(self) -> PipelineState:
        ...
        index_at = self._file_timestamp([VECTOR_INDEX_PATH, VECTOR_METADATA_PATH], use_oldest=True)
        model_at = self._file_timestamp([FUSION_MODEL_PATH], use_oldest=False)
