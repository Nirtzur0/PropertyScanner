...
class ImmoweltCrawlerAgent(BaseAgent):
    """
    Crawls Immowelt (Germany).
    """
            ...
            if html:
                try:
                    debug_path = self.scrape_client.build_raw_listing(
                        external_id=f"search_immowelt_{int(datetime.now().timestamp())}",
                        url=start_url,
                        html=html,
                        snapshot_ext="html"
                    )
                    logger.info("immowelt_search_snapshot_saved", path=debug_path)
                except Exception:
                    pass
