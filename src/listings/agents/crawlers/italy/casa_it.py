...
class CasaItCrawlerAgent(BaseAgent):
    """
    Crawls Casa.it.
    """
    def __init__(self, config: Dict[str, Any], compliance: ComplianceManager):
        super().__init__(name="CasaItCrawler", config=config)
        self.compliance = compliance
        self.source_id = "casa_it"
        self.base_url = config.get("base_url", "https://www.casa.it")
        rate_conf = config.get("rate_limit", {}) or {}
        self.rate_limit_seconds = float(rate_conf.get("period_seconds", config.get("period_seconds", 10)))
    ...
    def _fetch_url(self, url: str) -> Optional[str]:
        try:
            ...
            logger.warning("casait_fetch_error", url=url, error=str(e))
            return None

    def run(self, input_payload: Dict[str, Any]) -> AgentResponse:
        search_path = input_payload.get("search_path", "/vendita/residenziale/milano/") 
            ...
            if html:
                ...
                try:
                    debug_path = self.scrape_client.build_raw_listing(
                        external_id=f"search_casait_{unix_ts()}",
                        url=start_url,
                        html=html,
                        snapshot_ext="html"
                    )
                    logger.info("casait_search_snapshot_saved", path=debug_path)
                except Exception:
                    pass
                ...
                listing_urls = self.scrape_client.extract_links(
                    html,
                    LinkExtractorSpec(
                        selectors=["article a"], # Fallback selector, might need adjustment
                        include=["/immobile-"],
                    ),
                )
            else:
        ...
        for result in self.scrape_client.fetch_html_batch(listing_urls, timeout_s=30, retries=3):
            ...
            # Simple ID extraction logic for casa.it
            # typically casa.it urls are like /immobile-appartamento-milano-ID/
            try:
                # find numbers at the end or use hash
