...
class RealtorCrawlerAgent(BaseAgent):
    """
    Crawls Realtor.com (USA).
    """
    ...
    def _fetch_url(self, url: str) -> Optional[str]:
        try:
            ...
            logger.warning("realtor_fetch_error", url=url, error=str(e))
            return None

    def run(self, input_payload: Dict[str, Any]) -> AgentResponse:
        search_path = input_payload.get("search_path", "/realestateandhomes-search/San-Francisco_CA")
            ...
            if html:
                try:
                    debug_path = self.scrape_client.build_raw_listing(
                        external_id=f"search_realtor_{int(datetime.now().timestamp())}",
                        url=start_url,
                        html=html,
                        snapshot_ext="html"
                    )
                    logger.info("realtor_search_snapshot_saved", path=debug_path)
                except Exception:
                    pass
                ...
                listing_urls = self.scrape_client.extract_links(
                    html,
                    LinkExtractorSpec(
                        selectors=["a.card-anchor-link", "div.property-wrap a"], 
                        include=["/realestateandhomes-detail/"],
                    ),
                )
        ...
        for result in self.scrape_client.fetch_html_batch(listing_urls, timeout_s=45, retries=3):
            ...
            try:
                # Extract ID usually found in URL
