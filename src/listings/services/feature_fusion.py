...
from src.platform.settings import AppConfig, DescriptionAnalystConfig, ImageSelectorConfig, VLMConfig
...
class FeatureFusionService:
    ...
    def __init__(
        ...
        *,
        ...
        description_config: Optional[DescriptionAnalystConfig] = None,
        vlm_config: Optional[VLMConfig] = None,
        image_selector_config: Optional[ImageSelectorConfig] = None,
    ):
        if app_config is not None:
            if description_config is None:
                description_config = app_config.description_analyst
            if vlm_config is None:
                vlm_config = app_config.vlm
            if image_selector_config is None:
                image_selector_config = app_config.image_selector

        self.vlm = VLMImageDescriber(
            config=vlm_config,
            image_selector_config=image_selector_config,
        )
        self.analyst = DescriptionAnalyst(config=description_config)
