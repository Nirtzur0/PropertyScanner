...
from src.services.quality_gate import ListingQualityGate, QualityGateConfig, DataQualityError
...
class Harvester:
    def __init__(
        ...
        self.quality_gate = ListingQualityGate(quality_gate_config or QualityGateConfig())
    ...
    def run(self):
        ...
        tracker = PipelineRunTracker(db_path=str(DEFAULT_DB_PATH))
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.clean:
        ...
        if os.path.exists(CHECKPOINT_FILE_SALE):
            os.remove(CHECKPOINT_FILE_SALE)
        if os.path.exists(CHECKPOINT_FILE_RENT):
            os.remove(CHECKPOINT_FILE_RENT)
        if os.path.exists(STATE_FILE_SALE):
            os.remove(STATE_FILE_SALE)
        if os.path.exists(STATE_FILE_RENT):
            os.remove(STATE_FILE_RENT)
        if os.path.exists(SEEN_URLS_DB):
            os.remove(SEEN_URLS_DB)
