...
# ---------------------------------------------------------------------------
# Module-level helper functions (formerly @staticmethod on WorkbenchService)
# ---------------------------------------------------------------------------
...
def _valuation_payload(
    ...
    *,
    ...
    source_status_by_source_map: Optional[Dict[str, str]] = None,
    source_metrics_by_source_map: Optional[Dict[str, Dict[str, Any]]] = None,
) -> Dict[str, Any]:
    readiness = evaluate_valuation_readiness(row)
    cached = cached or persister.get_latest_valuation(row.id, max_age_days=None)
    if cached is not None:
        support = _clip(float(cached.confidence_score or 0.0))
    ...
    try:
        candidates = valuation_service._candidate_rows(
            target,
            k=3,
            source_status_by_source=source_status_by_source_map,
            source_metrics_by_source=source_metrics_by_source_map,
        )
...
def get_layers(reporting_service: ReportingService) -> Dict[str, Any]:
    return {
        "base_map": {
        ...
        "overlays": [
            ...
            },
            {
                ...
                "default": False,
                "blocked": len(reporting_service.list_benchmark_runs(limit=1)) == 0,
...
class ExploreService:
    ...
    def _query_rows(self, filters: WorkbenchFilters) -> tuple[int, List[DBListing]]:
        ...
        try:
            ...
            if filters.search:
                escaped = filters.search.strip().lower().replace("\\", "\\\\").replace("%", "\\%").replace("_", "\\_")
                like = f"%{escaped}%"
                query = query.filter(
                    func.lower(DBListing.title).like(like, escape="\\")
                    | func.lower(func.coalesce(DBListing.description, "")).like(like, escape="\\")
                )
    ...
    def explore(self, *, filters: WorkbenchFilters) -> Dict[str, Any]:
        ...
        source_map = _source_status_map(source_summary["sources"])
        ssbs = _source_status_by_source(source_summary["sources"])
        smbs = _source_metrics_by_source(source_summary["sources"])
        watchlists = self.workspace_service.list_watchlists()
        wl_index = _watchlist_index(watchlists)
        memo_idx = self._memo_index()
        comp_review_idx = self._comp_review_index()
        ...
        try:
            ...
            for row in rows:
                ...
                valuation = _valuation_payload(
                    ...
                    row,
                    persister,
                    source_status_by_source_map=ssbs,
                    source_metrics_by_source_map=smbs,
                )
                support = valuation["support"]
                if support is not None:
                    support_values.append(float(support))
                watchlisted = str(row.id) in wl_index
                memo_state = "drafted" if str(row.id) in memo_idx else "none"
                comp_review_state = "ready" if str(row.id) in comp_review_idx else "none"
                na = _next_action(
                    source_status=source_status,
                    support=support,
                    valuation_status=str(valuation["valuation_status"]),
                    has_comp_review=str(row.id) in comp_review_idx,
                    has_memo=str(row.id) in memo_idx,
                )
                marker = {
                    "id": row.id,
                    "title": row.title,
                    ...
                    "serving_reason": None,
                    "next_action": na,
                    "marker_color": _marker_color(source_status, support, str(valuation["valuation_status"])),
                    "marker_size": _marker_size(valuation["value_delta_pct"], valuation["deal_score"]),
                    "label": _format_price(float(row.price) if row.price is not None else None),
...
class ListingContextService:
    ...
    def _market_context(self, row: DBListing, cached: Any | None) -> Dict[str, Any]:
        ...
        return {
            ...
            "listed_at": _isoformat(row.listed_at),
            ...
            timeline.append(
                (
                    created_at,
                    {
                        ...
                        "title": f"Source contract {_labelize(str(item.get('status') or 'unknown'))}",
            ...
            timeline.append(
                (
                    created_at,
                    {
                        ...
                        "kind": "quality_event",
                        "title": _labelize(str(event.get("code") or "quality_event")),
        ...
        try:
            ...
            try:
                candidate_rows = self.valuation_service._candidate_rows(
                    target,
                    k=12,
                    source_status_by_source=ssbs,
                    source_metrics_by_source=smbs,
                )
            except ValueError as exc:
                data_gap = str(exc)
            readiness = evaluate_valuation_readiness(row)
