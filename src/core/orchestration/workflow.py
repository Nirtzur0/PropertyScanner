...
from src.cognitive.plan import build_default_plan, coerce_plan
...
class Orchestrator:
    ...
    def run_job(self, target_area: str = None, query: Optional[str] = None) -> Dict[str, Any]:
        if not target_area:
        ...
        return self.cognitive.run(query=query, areas=[target_area], plan=plan.model_dump(mode="json"))

    def run_batch(self, target_areas: List[str], max_workers: int = 3) -> List[Dict[str, Any]]:
        """
